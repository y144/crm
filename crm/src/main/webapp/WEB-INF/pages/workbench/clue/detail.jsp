...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveCreateRemarkBtn").click(function () {
			...
			var clueId='${clue.id}';//当本行js代码在浏览器执行时，${activity.id}已经变成32位字符串了。
			//表单验证
			if (noteContent==""){
				alert("备注内容不能为空");
				return;
			...
			$.ajax({
				...
				success:function (data) {
					if (data.code=="0"){
						...
						htmlStr+="<font color=\"gray\">线索</font> <font color=\"gray\">-</font> <b>${clue.company}</b> <small style=\"color: gray;\"> "+data.retData.createTime+" 由${sessionScope.sessionUser.name}创建</small>";
		...
		<c:forEach items="${remarkList}" var="remark">
			<div id="div_${remark.id}" class="remarkDiv" style="height: 60px;">
				...
				<div style="position: relative; top: -40px; left: 40px;" >
					...
					<font color="gray">线索</font> <font color="gray">-</font> <b>${clue.company}</b> <small style="color: gray;"> ${remark.editFlag==0?remark.createTime:remark.editTime} 由${remark.editFlag==0?remark.createBy:remark.editBy}${remark.editFlag==0?'创建':'修改'}</small>
